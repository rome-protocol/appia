...
/**
 * Appia — Stake, to the design system. Pay USDC from the wallet you already have → mSOL, a
 * self-custodied position. The journey ENGINE is unchanged (burn → delivered → fuel → swap → unwrap →
 * stake, every leg user-signed); only the render + the source chain change: no dropdown — the source is
 * the CONNECTED wallet's chain (isSupportedSource gates it). Live progress renders as the step legs
 * (the killed "road" is gone). Honest: no invented APY; yield accrues in the mSOL rate.
 */
...
import { ataBalance, awaitPdaCredit, pdaLamports, requestFuel, romeRpc, stakeMarinade, swapUsdcToWsol, unwrapWsol, watchDelivered } from "@/src/drivers";
...
const J_STORE = "appia:journey:v1";
type QuoteResp = {
...
const LEG_COPY: Record<string, { title: string; sub: string }> = {
  ...
  swap: { title: "Swapped to SOL", sub: "Meteora, at the rate you were quoted" },
  unwrap: { title: "Unwrapped", sub: "wSOL → native SOL, ready to stake" },
  stake: { title: "Staked with Marinade", sub: "mSOL minted to your own account — earning" },
};
...
export default function Stake() {
  ...
  const [position, setPosition] = useState<{ msol9: bigint } | null>(null);

  const src = chainId ?? 0;                       // source = the connected wallet's chain (no picker)
  ...
  const ready = isConnected && !!address && supportedSrc;                 // connected on a supported source → can stake
  const showForm = !running;                                             // the interface is always up except during an active run — clean page; recovery lives in Activity

  // Quote ALWAYS: a deterministic USDC→mSOL PREVIEW pre-connect / on an unsupported chain (pool + rate,
  // no wallet); the full route (bridge fee + ETA) once ready.
  useEffect(() => {
  ...
  const refreshPosition = useCallback(async () => {
    ...
    const msol = await ataBalance(romeSigner, ROME_CHAIN.msolMint);
    setPosition(msol > 0n ? { msol9: msol } : null);
  }, [romeSigner]);
  ...
  async function start() {
    ...
    let j = createJourney({ journey: "earn", terminal: "keep", sourceChainId: src, amountUsdc6 });
    persist(j);
    try {
      ...
      if (!registered) throw new Error("could not register the transfer with the bridge — your burn is safe and attested; retry from Activity to deliver it");
      ...
      const pdaBefore = await pdaLamports(romeSigner);
      const unwrapTx = await unwrapWsol(romeSigner);
      setLegTxs((p) => ({ ...p, unwrap: [unwrapTx] })); j = advance(j, { legId: "unwrap", txs: [unwrapTx] }); persist(j);
      const lam = await awaitPdaCredit(romeSigner, pdaBefore, BigInt(quote.minWsol9));
      const stakeTx = await stakeMarinade(romeSigner, lam - 5_000_000n);
      setLegTxs((p) => ({ ...p, stake: [stakeTx] })); j = advance(j, { legId: "stake", txs: [stakeTx] }); persist(j);
  ...
  return (
    <>
      <AppBar current="stake" />
      <div className="page action-cols">
        <div>
          <div className="eyebrow" style={{ marginBottom: 8 }}>Stake · Marinade liquid staking</div>
          <h2 className="serif" style={{ font: "500 28px var(--serif)", letterSpacing: "-.01em" }}>Stake SOL. Keep your keys.</h2>
          {ready
            ...
            : <p className="lede" style={{ marginTop: 14 }}>Pay USDC from an L2 you already hold → receive mSOL (a staking position) in your own account, self-custodied. Enter an amount to see the rate.</p>}

          {showForm && (
            <div className="action-card" style={{ marginTop: 16 }}>
              <div className="field"><div className="top"><label>You stake</label><span className="bal">USDC{ready ? " from your wallet" : ""}</span></div>
              ...
              <div className="field"><div className="top"><label>You receive · a position</label><span className="bal">to your own account</span></div>
                <div className="mid"><input value={quote ? fmt(BigInt(quote.estMsol9), 9) : "…"} readOnly aria-label="Amount in mSOL" /><span className="asset-select"><span className="cv sol">mS</span>mSOL</span></div></div>
              <div className="quote">
                {quote ? (<>
                  <div className="qr"><span className="k">Market</span><span className="v">Marinade · liquid staking on Solana</span></div>
                  <div className="qr"><span className="k">Exchange rate</span><span className="v earn mono">1 mSOL = {quote.solPerMsol} SOL, rising</span></div>
                  <div className="qr"><span className="k">Yield</span><span className="v muted">accrues in the mSOL rate — no fixed APY</span></div>
                  <div className="div" />
                  <div className="qr"><span className="k">Floor</span><span className="v mono">≥ {fmt(BigInt(quote.minWsol9), 9)} SOL staked, or nothing moves</span></div>
              ...
              {!isConnected ? (
                <button className="btn btn--primary btn--block" style={{ marginTop: 16 }} onClick={() => connectors[0] && connect({ connector: connectors[0] })}>
                  <span className="stack">Connect wallet to stake<small>pay USDC from a supported chain</small></span>
                </button>
              ) : !supportedSrc ? (
                ...
                <button className="btn btn--primary btn--block" style={{ marginTop: 16 }} onClick={start} disabled={!quote || running}>
                  <span className="stack">Stake {amount || "…"} USDC → mSOL<small>you approve each step · your keys throughout</small></span>
                </button>
              )}
              {!ready && (
          ...
          {running && journey && (
            <div className="action-card" style={{ marginTop: 16 }}>
              <div className="psec" style={{ marginTop: 0 }}><h3>{runErr ? "Paused" : "Staking your SOL…"}</h3><span className="c">{journey.legs.filter((l) => l.status === "done").length}/{journey.legs.length} · a few minutes</span></div>
              <div className="legs">
          ...
          {runErr && <div className="rec is-risk attn-card" style={{ marginTop: 14 }}><span className="ico">!</span><div className="tx"><b>The journey hit a snag.</b><div className="sub">{runErr} — your funds are recoverable from Activity.</div></div></div>}
        </div>

        <aside>
          <div className="psec" style={{ marginTop: 34 }}><h3>Your position</h3></div>
          {position ? (
            <div className="rec pos-card is-earn">
              <div className="ph"><span className="token sol">mSOL</span><div><div className="nm">Staked SOL</div><div className="pr">Marinade</div></div><span className="pill pill--earn"><span className="led" />Earning</span></div>
              <div className="fig">{fmt(position.msol9, 9)} <span className="u">mSOL</span></div>
              <div className="kv"><span className="k">Custody</span><span className="v">your own account</span></div>
            </div>
          ) : (
            <p className="dim" style={{ fontSize: 12 }}>Your staked mSOL will appear here and on the <a href="/">Dashboard</a>, and keeps earning until you bring it home.</p>
          )}
        </aside>
      </div>
