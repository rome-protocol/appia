...
/**
 * Shared app bar (the design system) — the Rome logomark + primary nav + wallet identity/chain
 * badge, self-managing its own wallet connect/disconnect. Used by every page so nav + branding stay
 * consistent. `current` sets the active nav item. Wallet = login; the connected chain is an inert
 * badge (no chain selector anywhere).
 */
...
export function AppBar({ current }: { current: "dashboard" | "swap" | "stake" | "lend" | "borrow" | "activity" }) {
  ...
  const cur = (id: string) => (current === id ? { "aria-current": "page" as const } : {});

  return (
    <header className="ab">
      <span className="ab__brand"><Logomark /><span className="name">Appia</span></span>
      <nav className="ab__nav" aria-label="Primary">
        <a className="navlink" {...cur("dashboard")} href="/">Dashboard</a>
        <a className="navlink" {...cur("swap")} href="/swap">Swap</a>
        <a className="navlink" {...cur("stake")} href="/earn">Stake</a>
        <a className="navlink" {...cur("lend")} href="/lend">Lend</a>
        <a className="navlink" {...cur("borrow")} href="/borrow">Borrow<span className="soon">soon</span></a>
        <a className="navlink" {...cur("activity")} href="/claims">Activity</a>
      </nav>
      <div className="ab__right">
        {isConnected ? (
          <span className="wallet">
            <span className="addr">{address?.slice(0, 6)}…{address?.slice(-4)}</span>
            {chainName && <span className="chain-badge"><span className="dot" /> {chainName}</span>}
            <button className="btn-disc" onClick={() => disconnect()} title="Disconnect">Disconnect</button>
          </span>
        ) : (
